...
const ResultTable = ({plan, plan_pas,competence_scales,indicator_scales,question_scales,...props}) => {
    ...
    //selfCompetence_appraisalArray
    let selfCompetence_appraisalArray = selfPlan_pas.filter(el=>el.assessment_appraise_type === 'competence_appraisal')[0].competences.competence.map(item =>{
        //score={2} comment={'hf'} bossScore={8} bossComment={'jdfjdf'}
        //competence_scales.find(c=>c.id === item.competence_id).//name
...
console.log(selfPlan_pas);
console.log(bossPlan_pas);
console.log(competence_scales);
console.log(selfCompetence_appraisalArray)
    return (
